...
function TailoringSequence() {
  ...
  useEffect(() => {
    ...
    const syncFrame = () => {
      ...
      renderedTime += (targetTime - renderedTime) * 0.32;

      if (Math.abs(video.currentTime - renderedTime) > 0.006) {
        video.currentTime = Math.max(
          0,
          Math.min(renderedTime, Math.max(video.duration - 0.025, 0)),
        );
      }
    ...
    const trigger = ScrollTrigger.create({
      ...
      onUpdate(self) {
        ...
        const duration = Math.max(video.duration - 0.025, 0);
  ...
  return (
    <div ref={rootRef} className="tailoring-scroll-video">
      <div className="tailoring-video-glow" aria-hidden="true" />
      <video
        ref={videoRef}
        className="tailoring-source-video"
        src="/anatomia/renan-scroll.mp4"
        muted
        playsInline
        preload="auto"
        disablePictureInPicture
        controls={false}
        aria-label="Construção do traje do advogado controlada pela rolagem"
      />
      <div className="tailoring-video-line" aria-hidden="true" />
    </div>
  );
}
